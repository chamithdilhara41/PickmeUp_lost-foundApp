...
const HomeScreen = () => {
  ...
  const { user } = useAuth();
  const [allLostItems, setAllLostItems] = useState<Lost[]>([]);
  const [recentLostItems, setRecentLostItems] = useState<Lost[]>([]);
  const [userLostItems, setUserLostItems] = useState<Lost[]>([]);
  ...
  const loadData = () => {
    ...
    // Load all items first, then filter client-side (temporary workaround)
    const allItemsQuery = query(lostRef, orderBy("createdAt", "desc"));

    const unsubscribeAll = onSnapshot(allItemsQuery, (snapshot) => {
      ...
      // Filter for lost status client-side
      const lostItems = items.filter(item => item.status === "lost");
      setRecentLostItems(lostItems.slice(0, 6));
      setStats(prev => ({ ...prev, totalLost: lostItems.length }));

      // Filter user's items if logged in
      if (user) {
        const userItems = items.filter(item => item.userId === user.uid);
        setUserLostItems(userItems.slice(0, 3));
        setStats(prev => ({ 
          ...prev, 
          userItems: userItems.length,
          totalFound: userItems.filter(item => item.status === "found").length
        }));
      }
    ...
    >
      ...
      <View className="bg-white px-6 pt-12 pb-6">
        ...
        <View className="bg-blue-50 rounded-2xl p-6">
          <Text className="text-xl font-semibold text-gray-800 mb-2">
            {user ? `Welcome back, ${user.email?.split('@')[0]}!` : "Welcome to PickMeUp"}
          </Text>
          <Text className="text-gray-600">
            {user 
              ? "Help others find their lost items or report your own."
              : "Sign in to report lost items and help others."
            }
          </Text>
        </View>
      </View>
      ...
      <View className="px-6 mt-6">
        ...
        <View className="flex-row justify-between">
          <TouchableOpacity
            ...
            className="bg-white rounded-2xl p-5 flex-1 mr-3 shadow-sm items-center"
          >
            ...
            <Text className="text-gray-800 font-medium text-center">Browse Lost Items</Text>
          </TouchableOpacity>

          <TouchableOpacity
            onPress={() => router.push(user ? "/(dashboard)/lost/new" : "/login")}
            className="bg-white rounded-2xl p-5 flex-1 ml-3 shadow-sm items-center"
          >
            ...
            <Text className="text-gray-800 font-medium text-center">Report Lost Item</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      <View className="px-6 mt-6 mb-8">
        <View className="flex-row justify-between items-center mb-4">
          <Text className="text-xl font-semibold text-gray-800">Recently Lost</Text>
        ...
        {recentLostItems.length === 0 ? (
          <View className="bg-white rounded-2xl p-8 items-center">
            ...
            <Text className="text-gray-500 mt-3 text-center">No lost items reported yet</Text>
            <Text className="text-gray-400 text-center mt-1">Be the first to report a lost item</Text>
          </View>
        ) : (
      ...
      {/* User's Items (if logged in) */}
      {user && userLostItems.length > 0 && (
        <View className="px-6 mt-2 mb-8">
          <View className="flex-row justify-between items-center mb-4">
            <Text className="text-xl font-semibold text-gray-800">Your Items</Text>
                  ...
                  <View className="flex-row items-center mt-1">
                    <View className={`px-2 py-1 rounded-full ${
                      ...
                      <Text className={`text-xs ${
                        item.status === "found" ? "text-green-800" : "text-red-800"
                      }`}>
      ...
      {!user && (
        <View className="px-6 mt-4 mb-8">
          <View className="bg-blue-50 rounded-2xl p-6">
            <Text className="text-lg font-semibold text-gray-800 mb-2">Join Our Community</Text>
            <Text className="text-gray-600 mb-4">
              Sign in to report lost items, help others, and get notified when your items are found.
            </Text>
            <TouchableOpacity
              onPress={() => router.push("/login")}
              className="bg-blue-600 px-6 py-3 rounded-xl"
            >
              <Text className="text-white font-semibold text-center">Sign In / Register</Text>
            </TouchableOpacity>
          </View>
        </View>
      )}
