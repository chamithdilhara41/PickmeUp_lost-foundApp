...
import { createFound, getFoundById, updateFound } from "@/services/foundService";
...
const FoundFormScreen = () => {
  ...
  const [address, setAddress] = useState<string>(""); // ✅ Added: Where found (detailed address)
  ...
  useEffect(() => {
    const load = async () => {
      if (!isNew && id) {
        try {
          showLoader();
          const found = await getFoundById(id);
          if (found) {
            setTitle(found.title);
            setDescription(found.description);
            setLocation(found.location ?? "");
            setAddress(found.address ?? ""); // ✅ Load address if exists
            setCategory(found.category ?? "");
            setPhone(found.phone ?? "");
            setEmail(found.email ?? user?.email ?? "");
            setImages(found.serverImageUrls ?? []);
          }
        } finally {
          hideLoader();
        }
  ...
  const validateField = (name: string, value: string) => {
    ...
    switch (name) {
      ...
      case "location":
        ...
        break;
      case "address":
        if (!value.trim()) error = "Please specify where you found this item";
        break;
      case "category":
  ...
  const validateForm = () => {
    const fields = [
      ...
      { name: "address", value: address }, // ✅ Added address validation
    ...
    try {
      ...
      if (isNew) {
        await createFound({
          ...
          serverImageUrls,
          userId: user?.uid ?? "",
          status: "found", // ✅ Changed status to "found"
        });
        Alert.alert("Success", "Found item added successfully!");
      } else {
        await updateFound(id!, {
          title,
          description,
          location,
          address, // ✅ Added address field
        ...
        Alert.alert("Success", "Found item updated successfully!");
      }

      router.back();
    } catch (err) {
      console.error(`Error ${isNew ? "saving" : "updating"} found item`, err);
  ...
  return (
    <ScrollView className="flex-1 bg-gray-50 p-5">
      <Text className="text-3xl font-bold text-green-800 mb-6 text-center">
        {isNew ? "Report Found Item" : "Edit Found Item"}
      </Text>
      ...
      {/* ✅ Where Found (Address) */}
      <View className="mb-4">
        <Text className="text-sm font-semibold text-gray-700 mb-2">Where Found *</Text>
        <TextInput
          placeholder="Enter specific location where you found this item (e.g., Galle Face Green, near the fountain)"
          multiline
          numberOfLines={3}
          className={`border p-4 rounded-lg bg-white min-h-20 ${errors.address ? "border-red-500" : "border-gray-300"}`}
          value={address}
          onChangeText={(text) => {
            setAddress(text);
      ...
      {/* Submit Button */}
      <TouchableOpacity
        className={`bg-green-600 rounded-lg p-4 mb-8 flex-row justify-center items-center ${isSubmitting ? "opacity-70" : ""}`}
      ...
      >
        {isSubmitting ? (
          ...
          <Text className="text-white text-lg font-semibold">
            {isNew ? "Add Found Item" : "Update Found Item"}
          </Text>
        )}
      </TouchableOpacity>
    </ScrollView>
...
export default FoundFormScreen;